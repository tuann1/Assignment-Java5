...
<section>
    <div class="container">
        <form action="/nsx/search/${currentPage}" method="get">
    ...
    <table class="table">
        ...
        <tbody>
        <c:forEach items="${listNsx}" var="n" varStatus="viTri">
            <tr>
                <td>${n.id}</td>
                <td>${n.ma}</td>
                <td>${n.ten}</td>
                <td>
                    <a href="/nsx/view-update/${n.id}" tabindex="-1"
                       role="button" class="btn btn-primary"
                       aria-disabled="true">Update</a>
                    <a href="/nsx/remove/${n.id}" tabindex="-1"
    ...
    <div>
        ...
        <span>
                <a href="/nsx/hien-thi/${i}">${i}</a>
        </span> &nbsp;&nbsp;
        </c:forEach>

        <c:choose>
            <c:when test="${currentPage < totalPages}">
                <a href="/nsx/hien-thi/${currentPage + 1}">Next</a>
            </c:when>
            <c:otherwise>
                <span>Next</span>
            </c:otherwise>
        ...
        <c:choose>
            <c:when test="${currentPage > 1}">
                <a href="/nsx/hien-thi/${currentPage - 1}">Previous</a>
            </c:when>
            <c:otherwise>
                <span>Previous</span>
            </c:otherwise>
