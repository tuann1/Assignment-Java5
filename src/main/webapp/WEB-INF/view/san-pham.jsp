...
<section>
    <div class="container">
        <form action="/san-pham/search/${currentPage}" method="get">
    ...
    <table class="table">
        ...
        <tbody>
        <c:forEach items="${listSanPham}" var="sanPham" varStatus="viTri">
            <tr>
                <td>${sanPham.id}</td>
                <td>${sanPham.ma}</td>
                <td>${sanPham.ten}</td>
                <td>
                    <a href="/san-pham/view-update/${sanPham.id}" tabindex="-1"
                       role="button" class="btn btn-primary"
                       aria-disabled="true">Update</a>
                    <a href="/san-pham/remove/${sanPham.id}" tabindex="-1"
    ...
    <div>
        ...
        <span>
                <a href="/san-pham/hien-thi/${i}">${i}</a>
        </span> &nbsp;&nbsp;
        </c:forEach>

        <c:choose>
            <c:when test="${currentPage < totalPages}">
                <a href="/san-pham/hien-thi/${currentPage + 1}">Next</a>
            </c:when>
            <c:otherwise>
                <span>Next</span>
            </c:otherwise>
        ...
        <c:choose>
            <c:when test="${currentPage > 1}">
                <a href="/san-pham/hien-thi/${currentPage - 1}">Previous</a>
            </c:when>
            <c:otherwise>
                <span>Previous</span>
            </c:otherwise>
